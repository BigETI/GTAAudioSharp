                    ...
                    if (Directory.Exists(directory))
                    {
                            ...
                            Directory.Exists(streams_directory))
                        {
                                ...
                                File.Exists(trak_lookup_dat_path))
                            {
                                ...
                                using (FileStream stream = File.Open(stream_paks_dat_path, FileMode.Open))
                                {
                                    long stream_length = stream.Length;
                                ...
                                using (FileStream stream = File.Open(bank_lookup_dat_path, FileMode.Open))
                                {
                                ...
                                for (int i = 0; i < sfx_files.Length; i++)
                                {
                                    ...
                                    if (sfx_file != null)
                                    {
                                        ...
                                        {
                                            stream = File.Open(sfx_path, FileMode.Open);
                                            if (stream != null)
                                            {
                                                ...
                                                if (reader != null)
                                                {
                                                    ...
                                                    if (num_audio <= maxSoundEntries)
                                                    {
                                                        ...
                                                        for (uint j = 0U; j != num_audio; j++)
                                                        {
                                                            ...
                                                            data[j] = new GTAAudioSFXDataInfo(offset, sample_rate);
                                                        }
                                                    }
                                                }
                                            }
                                    ...
                                    sfx_audio_files[i] = new GTAAudioSFXFile(sfx_file, stream, sfx_lookup[i].ToArray(), data);
                                    sfx_lookup[i].Clear();
                                }
                                for (int i = 0; i < streams_files.Length; i++)
                                {
                                    ...
                                    if (streams_file != null)
                                    {
                                        ...
                                        {
                                            stream = File.Open(streams_path, FileMode.Open);
                                            if (stream != null)
                                            {
